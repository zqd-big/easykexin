 ...
 * 4.用户使用# VOS_PriQuePushBatch添加数据时，优先队列无法感知用户数据类型，必须注册dataFunc->dupFunc。
 ...
 * 2.当队列为空时，接口返回0，该接口无法识别返回的值是错误码还是用户数据，用户使用前必须进行#VOS_PriQueEmpty判断是否为空。\n
 * @param priQueue [IN] priority queue控制块。
 * @retval 用户数据。
 * @par 依赖：无
 * @li vos_priorityqueue.h：该接口声明所在的文件。
